...
pub fn get_settings(conn: &Connection) -> SqliteResult<Settings> {
    ...
    if let Some(row) = rows.next()? {
        ...
        Ok(Settings {
            ...
            default_research_depth: "light".to_string(),
            apollo_enabled: false,
            apollo_max_contacts: 10,
            deep_job_concurrency: 1,
...
pub fn update_settings(conn: &Connection, use_chrome: bool) -> SqliteResult<()> {
    let now = chrono::Utc::now().timestamp_millis();
    eprintln!("[db] Executing UPDATE settings: use_chrome={}", use_chrome);
...
pub fn update_orchestration_settings(
    ...
    conn.execute(
        "UPDATE settings SET
            ...
            apollo_enabled = ?3,
            apollo_max_contacts = ?4,
            deep_job_concurrency = ?5,
            daily_budget_usd_cents = ?6,
            updated_at = ?7
         WHERE id = 1",
        params![
