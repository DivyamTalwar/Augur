...
export function LeadResearchPanel({
  ...
  const handleStartResearch = async () => {
    await submitResearch(async () => {
      ...
      const result = await startResearch(lead.id, handleStreamEvent, undefined, researchDepth);
  ...
  return (
    <div className="space-y-2">
      {people.map((person) => {
        ...
        const statusColors = {
          pending: "bg-bg-2",
          in_progress: "bg-flame/15",
          completed: "bg-leaf/15",
        ...
        return (
          <Link
            key={person.id}
            to={`/people/${person.id}`}
            className="flex items-center gap-4 p-3 rounded-lg border border-line hover:bg-bg-2 hover:border-line transition-colors cursor-pointer"
          >
            <div
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              {person.email && (
                <span
                  ...
                  className="p-1.5 rounded hover:bg-bg-2 text-muted-foreground hover:text-foreground transition-colors"
                  title={person.email}
                >
