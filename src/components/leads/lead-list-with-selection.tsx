...
import { IconChevronRight, IconSearch, IconTrash, IconChartBar } from "@/components/ui/icon";
...
export function LeadListWithSelection({ groupedLeads, onRefresh }: LeadListWithSelectionProps) {
  ...
  const handleScore = React.useCallback(
    async (selectedIds: number[]) => {
      ...
      if (queued > 0) {
        toast.success(`Scoring ${queued} lead${queued === 1 ? "" : "s"} — watch the dock.`);
      }
      if (skipped > 0) {
        toast.message(
          `${skipped} lead${skipped === 1 ? "" : "s"} skipped`,
          { description: "Run research first — scoring needs an enriched profile." }
        );
      }
      if (failed > 0) {
        toast.error(`${failed} lead${failed === 1 ? "" : "s"} failed to queue`);
      }
    },
    [leadMap]
  );
  ...
  return (
    <SelectableRow id={lead.id}>
      ...
      <div className="flex items-center gap-2 shrink-0">
        {lead.industry && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-bg-2 text-muted-foreground">
            {lead.industry}
          </span>
        )}
