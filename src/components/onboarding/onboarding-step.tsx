...
import { IconCircleCheck, IconCircle, IconCircleDot } from "@/components/ui/icon";
...
export function OnboardingStep({ step, isActive, onClick }: OnboardingStepProps) {
  return (
    <div className={`relative ${isActive ? "border-l-2 border-primary -ml-0.5" : ""}`}>
      <button
        onClick={onClick}
        className="flex items-center gap-2 w-full px-2 py-1 text-left rounded hover:bg-bg-2 transition-colors"
      >
        {step.isCompleted ? (
          <motion.div
            key="completed"
