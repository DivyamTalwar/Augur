...
import { IconChevronDown, IconChevronRight, IconPlus } from "@/components/ui/icon";
...
export function CollapsibleStatusGroup({
  ...
  return (
    <div className="group/status">
      <div className="sticky top-0 bg-bg backdrop-blur-sm z-10 flex items-center gap-2 px-3 py-2 text-sm border-b border-line">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="w-4 shrink-0 flex items-center justify-center hover:bg-bg-2 transition-colors"
        >
          {isOpen ? (
        ...
        <button className="p-1 hover:bg-bg-2 opacity-0 group-hover/status:opacity-100 transition-opacity">
