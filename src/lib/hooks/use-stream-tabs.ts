...
/**
 * Decide whether a job should still appear as a dock tab.
 *  - running / queued                        → always
 *  - completed / error / timeout / cancelled → only for 40 minutes after completion
 *
 * Prevents stale tabs from prior sessions cluttering the dock.
 */
const FRESH_WINDOW_MS = 40 * 60 * 1000;
function isFreshDockJob(job: Job, now: number): boolean {
  if (job.status === "running" || job.status === "queued") return true;
  const finishedAt = job.completedAt ?? job.startedAt ?? job.createdAt;
  return now - finishedAt < FRESH_WINDOW_MS;
}

function filterAndMapJobs(jobs: Job[]): StreamTab[] {
  const now = Date.now();
  return jobs
    .filter((job) => isFreshDockJob(job, now))
    .map(mapJobToTab)
...
export function useStreamTabs(limit: number = 50) {
  ...
  // Filter is cheap (≤50 jobs) so we recompute every render rather than
  // memoising — that lets `Date.now()` evaluate naturally and avoids the
  // react-hooks/purity rule rejecting Date.now() inside useMemo.
  const tabs = filterAndMapJobs(jobs);
